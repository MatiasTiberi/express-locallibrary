...
//Display datail page for a specific BookInstance
...
exports.bookinstance_create_get = function(req, res, next){
    ...
    Book.find({}, 'title')
        .exec(function(err, books) {
            if (err) {return next(err);}
            //Successful, so render
            res.render('bookinstance_form', {title: 'Create BookInstace', book_list: books});
        });
};
...
exports.bookinstance_delete_post = function(req, res, next) {
    ...
    async.parallel({
        ...
        if (err) {return next(err);}
        BookInstance.findByIdAndRemove(req.body.bookinstanceid, function deletebookinsntace(err) {
            if (err) {return next(err);}
            res.redirect('/catalog/bookinstances');
        });
    });
...
exports.bookinstance_update_get = function(req, res, next) {
    ...
    async.parallel({
        ...
        if (err) {return next(err);}
        if (results.bookinstance==null) {
            var err = new Error('Error BookInstance not found');
            err.status = 404;
            return next(err);
        }
...
exports.bookinstance_update_post = [
    ...
    validator.body('book', 'Book must not be empty').trim().isLength({min:1}),
    validator.body('imprint', 'Imprint must not be empty').trim().isLength({min:1}),
    validator.body('due_back', 'Invalid date').optional({checkFalsy: true}).isISO8601(),
        ...
        if (!errors.isEmpty()) {
            ...
            Book.find({}, 'title')
                .exec(function(err, books) {
                    ...
                    res.render('bookinstance_form', {title: 'Update BookInstance', bookinstance: bookinstance, errors: errors.array(), selected_book: bookinstance.book_id});
                });
            return;
        }
        else {
